...
<div class="container-fluid p-3 m-4 h-100" style="background-color: #FFFFFF; border-top: 5px solid #D40013; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);" >
    ...
    <hr>
        <div class="container-fluid"> 
            <form class="row g-3">
                <input id="id" name="id" value = <?php echo @$user["id"]?> type="hidden" disabled readonly>
                <div class="col-md-12">
                  <label for="name" class="form-label">Name</label>
                  <input class="form-control" id="name" name="name" value=<?php echo @$user["name"]?> aria-describedby="emailHelp" disabled readonly>
                </div>
                <div class="col-md-12">
                  <label for="exampleInputEmaill" class="form-label">Phone Number</label>
                  <input class="form-control" id="phonenum" name="phonenum" value=<?php echo @$user["phonenum"]?> aria-describedby="emailHelp" disabled readonly>
                </div>
                <div class="col-md-12">
                  <label for="exampleInputEmaill" class="form-label">Email</label>
                  <input class="form-control" id="email" name="email" value=<?php echo @$user["email"]?> aria-describedby="emailHelp" disabled readonly>
                </div>
                <div class="col-md-12">
                    <label for="job" class="form-label">Job</label>
                    <select class="form-control" id="job" onchange="getSelectValue(this.value);" disabled readonly>
                ...
                <div class="d-grid gap-2">
                    <button class="btn btn-primary btn-dark" type="submit" name="update" disabled>Save</button>
                </div>
              </form>
        </div>         
</div>
 ...
 <script type="text/javascript">
    function getSelectValue(job){
        if(job!=''){
            $("#job option[value='"+<?php echo @$user["job"];?>+"']").hide();
            //alert(job);
        }
    }
 </script>
